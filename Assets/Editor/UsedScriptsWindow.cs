...
public class UsedScriptsWindow : EditorWindow
{
    ...
    private Dictionary<string, float> lastClickTimes;
    ...
    {
        RefreshScriptsList();
        lastClickTimes = new Dictionary<string, float>();
    }
    ...
    private void OnGUI()
    {
        ...
        foreach (var entry in scriptToGameObjects)
        {
            ...
            // Detect double-click
            if (GUILayout.Button(entry.Key, EditorStyles.foldoutHeader))
            {
                if (lastClickTimes.ContainsKey(entry.Key) && (Time.realtimeSinceStartup - lastClickTimes[entry.Key] < 0.3f))
                {
                    SelectAllGameObjectsWithScript(entry.Key);
                }
                lastClickTimes[entry.Key] = Time.realtimeSinceStartup;
            }
    ...
    private void SelectAllGameObjectsWithScript(string scriptName)
    {
        if (scriptToGameObjects.ContainsKey(scriptName))
        {
            Selection.objects = scriptToGameObjects[scriptName].ToArray();
        }
    }
}
